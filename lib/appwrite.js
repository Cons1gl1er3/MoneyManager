...
export const getTransactions = async (userID, forceRefresh = false) => {
	try {
	  console.log(`Getting transactions for user: ${userID}, forceRefresh: ${forceRefresh}`);
	  ...
	  console.log(`Raw transactions fetched: ${transactions.documents.length}`);
	  ...
	  if (invalidTransactions.length > 0) {
		console.log(`Found ${invalidTransactions.length} transactions with null relations:`, invalidTransactions.map(t => ({ id: t.$id, account_id: t.account_id, category_id: t.category_id, user_id: t.user_id })));
	  }
	  ...
	  console.log(`Mapped transactions: ${transactionsMapped.length}`);
...
export const updateTransaction = async (transactionID, updates) => {
	try {
	  console.log('Backend: Starting updateTransaction');
	  console.log('Backend: Transaction ID:', transactionID);
	  ...
	  console.log('Backend: Original transaction retrieved:', originalTransaction);
	  ...
	  console.log('Backend: Transaction document updated:', updatedTransaction);
	  ...
	  console.log('Backend: Change detection - amount:', amountChanged, 'account:', accountChanged, 'income type:', incomeTypeChanged);
  ...
  export const deleteTransaction = async (transactionID) => {
	try {
	  ...
	  console.log('Retrieved transaction for deletion:', transaction);
  
	  	  // Extract the account ID correctly from the relationship
	  const accountID = transaction.account_id?.$id || transaction.account_id;
	  
	  console.log('Reversing account balance for account ID:', accountID, 'amount:', transaction.amount, 'was income:', transaction.is_income);
