...
export const createUser = async (email, password, username) => {
	try {
		...
		// Create default "Tiền mặt" account for new user
		await createAccount('Tiền mặt', 0);
...
export const getTransactions = async (userID, forceRefresh = false) => {
	...
	while (retries <= MAX_RETRIES) {
		try {
			...
			const LIMIT = 100; // Maximum documents per request
			let offset = 0;
			let allTransactions = [];
			let hasMore = true;
			...
			while (hasMore) {
				...
				// Check if we've received fewer documents than the limit
			...
			console.log(`Backend: Raw transactions fetched: ${allTransactions.length}`);

			// Log any transactions with null relations for debugging
			const invalidTransactions = allTransactions.filter(doc => 
			...
			const transactionsMapped = allTransactions
				.filter(doc => {
...
export const uploadUserAvatar = async (file) => {
  try {
    ...
    let fileData = file;
    if (file instanceof File && file[0]?.startsWith('data:')) {
      const base64Data = file[0].split(',')[1];
      ...
      fileData = new Blob(byteArrays, { type: 'image/jpeg' });
    }
    
    // Upload the file to Appwrite Storage
    console.log('Uploading to storage bucket:', config.storageId);
    ...
    console.error('Detailed error in uploadUserAvatar:', error);
    if (error.message?.includes('File not found in payload')) {
...
export const changeUserPassword = async (newPassword, oldPassword) => {
  try {
